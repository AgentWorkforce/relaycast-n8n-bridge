 ...
 */
export function loadCustomSteps(cwd) {
    ...
    try {
        ...
        if (!config || typeof config !== 'object') {
            throw new CustomStepsParseError('Invalid file format', 'The file must contain a valid YAML object with a "steps" key', stepsPath);
        }
        if (!config.steps) {
            throw new CustomStepsParseError('Missing "steps" key', 'Add a "steps" object containing your custom step definitions:\n\n' +
                'steps:\n' +
                '  my-step:\n' +
                '    command: "echo hello"', stepsPath);
        }
        if (typeof config.steps !== 'object' || Array.isArray(config.steps)) {
            throw new CustomStepsParseError('Invalid "steps" format', 'The "steps" key must be an object (not an array) mapping step names to definitions', stepsPath);
        }
...
export class CustomStepsParseError extends Error {
    ...
    suggestion;
    filePath;
    constructor(issue, suggestion, filePath) {
...
export class CustomStepResolutionError extends Error {
    ...
    issue;
    suggestion;
    constructor(stepName, issue, suggestion) {
 ...
 */
function validateCustomStepDefinition(name, def, filePath) {
    if (!def || typeof def !== 'object') {
        throw new CustomStepsParseError(`Invalid step "${name}"`, 'Each step must be an object with at least a "command" or "branch" field:\n\n' +
            `steps:\n` +
            `  ${name}:\n` +
            `    command: "your-command-here"`, filePath);
    }
    const stepDef = def;
    // Validate type if specified
    if (stepDef.type !== undefined) {
        if (stepDef.type !== 'deterministic' && stepDef.type !== 'worktree') {
            throw new CustomStepsParseError(`Invalid type "${stepDef.type}" for step "${name}"`, 'Step type must be either "deterministic" or "worktree"', filePath);
        }
    }
    ...
    const explicitType = stepDef.type;
    const stepType = explicitType ?? (hasCommand ? 'deterministic' : hasBranch ? 'worktree' : undefined);
    if (!stepType) {
        throw new CustomStepsParseError(`Step "${name}" is missing required fields`, 'Deterministic steps need "command", worktree steps need "branch":\n\n' +
            '# Deterministic step:\n' +
            `  ${name}:\n` +
            ...
            `  ${name}:\n` +
            '    type: worktree\n' +
            '    branch: "{{branch-name}}"', filePath);
    }
    if (stepType === 'deterministic' && !hasCommand) {
        throw new CustomStepsParseError(`Deterministic step "${name}" is missing "command"`, 'Add a command field:\n\n' +
            `  ${name}:\n` +
            '    command: "your-shell-command"', filePath);
    }
    if (stepType === 'worktree' && !hasBranch) {
        throw new CustomStepsParseError(`Worktree step "${name}" is missing "branch"`, 'Add a branch field:\n\n' +
            `  ${name}:\n` +
            '    type: worktree\n' +
            '    branch: "feature/{{branch-name}}"', filePath);
    }
    // Validate params if present
    if (stepDef.params !== undefined) {
        if (!Array.isArray(stepDef.params)) {
            throw new CustomStepsParseError(`Invalid params for step "${name}"`, 'Params must be an array:\n\n' +
                `  ${name}:\n` +
                '    params:\n' +
                '      - name: myParam\n' +
                '        required: true\n' +
        ...
        for (let i = 0; i < stepDef.params.length; i++) {
            const param = stepDef.params[i];
            if (!param || typeof param !== 'object') {
                throw new CustomStepsParseError(`Invalid param at index ${i} for step "${name}"`, 'Each param must be an object with at least a "name" field', filePath);
            }
            if (!param.name || typeof param.name !== 'string') {
                throw new CustomStepsParseError(`Param at index ${i} for step "${name}" is missing "name"`, 'Add a name to the parameter:\n\n' +
                    '    params:\n' +
                    '      - name: myParam', filePath);
            }
            if (param.required !== undefined && typeof param.required !== 'boolean') {
                throw new CustomStepsParseError(`Invalid "required" value for param "${param.name}" in step "${name}"`, 'The "required" field must be true or false', filePath);
            }
        }
    }
}
 ...
 */
export function validateCustomStepsUsage(steps, customSteps) {
    const errors = [];
    const warnings = [];
    const missingSteps = [];
    const missingParams = [];
    ...
    for (const step of steps) {
        ...
        if (!customDef) {
            ...
            errors.push(`Step "${step.name}" uses undefined custom step "${step.use}". ` +
        ...
        if (customDef.params) {
            for (const param of customDef.params) {
                ...
                if (providedValue !== undefined) {
                    ...
                    errors.push(`Step "${step.name}" is missing required parameter "${param.name}" for custom step "${step.use}".`);
                }
            }
        }
        ...
        for (const variable of variables) {
            if (!providedParams.has(variable)) {
                ...
                if (!paramDef) {
                    ...
                    warnings.push(`Step "${step.name}": Variable "{{${variable}}}" in custom step "${step.use}" ` +
        ...
        const stepKeys = Object.keys(stepAny).filter((k) => !['name', 'use', 'dependsOn', 'timeoutMs'].includes(k));
        for (const key of stepKeys) {
            if (!definedParams.has(key)) {
                warnings.push(`Step "${step.name}": Parameter "${key}" is not defined in custom step "${step.use}" and will be ignored.`);
            }
        }
    }
    return {
 ...
 */
export function resolveCustomStep(step, customSteps) {
    if (!step.use) {
        return step;
    }
    const customDef = customSteps.get(step.use);
    if (!customDef) {
        throw new CustomStepResolutionError(step.name, `Custom step "${step.use}" not found`, `Make sure "${step.use}" is defined in .relay/steps.yaml:\n\n` +
            'steps:\n' +
            `  ${step.use}:\n` +
